<template>
  ...
  <div>
    ...
    <table
      ...
      v-if="item in $store.state.product"
      :key.id
    >
      <tr>
      ...
      <tr v-for="item in $store.state.product" :key="item.id">
        <td style="color: white; background-color: indigo; padding-top: 40px">
          {{ item.prodID }}
        </td>
        <td
          style="
        ...
        <td style="color: white; background-color: indigo">
          <img id="adminImg" :src="item.prodUrl" />
        </td>
        <td
          style="
            color: white;
        ...
        <td>
          <button @click="deleteProd(item.prodID)" id="deleteButt">
            Delete
          </button>
        </td>
        ...
        <td>
          <button
            ...
            class="btn btn-primary"
            data-bs-toggle="modal"
            :data-bs-target="'#exampleModal2-' + item.prodID"
            id="editButt"
          >
            Edit
          </button>
...
export default {
  ...
  methods: {
    ...
    deleteProd(id) {
      this.$store.dispatch("deleteProd", id).then(() => {
        sweet({
          title: "Are you sure?",
    ...
    updateProd(id) {
      ...
      this.$store.dispatch("updateProd", edit).then(() => {
